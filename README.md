...
**Domain:** `Ops / Incident Management` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Incident Report Gen** is a standalone, browser-based AI workflow app for **Ops / Incident Management**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `incident_notes`<br>Incident notes | textarea | Yes | — | Affects input: Incident notes. |
...
| `incident_type`<br>incident type | text | No | — | Affects customization: incident type. |
| `severity_model`<br>severity model | select | No | CVSS | Affects customization: severity model. |
| `audience`<br>audience | select | No | general | Affects customization: audience. |
| `company_template`<br>company template | text | No | — | Affects customization: company template. |
| `timezone`<br>timezone | text | No | — | Affects customization: timezone. |
| `blameless_tone`<br>blameless tone | text | No | — | Affects customization: blameless tone. |
| `required_fields`<br>required fields | text | No | — | Affects customization: required fields. |
...
- **severity model**: CVSS, business impact, low/medium/high, custom
- **audience**: general, executive, technical, legal, medical, student, customer
...
uvicorn app.main:app --host 127.0.0.1 --port 9126
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9126`. |
...
```bash
curl http://127.0.0.1:9126/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9126/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9126 uvicorn app.main:app --host 0.0.0.0 --port 9126
```
...
- `plugins/incident-report-gen.json` for plugin metadata.
